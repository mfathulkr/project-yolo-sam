...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare a source-scene-safe SAMRS SOTA small-vehicle master corpus."
    )
    parser.add_argument(
        "--protocol",
        type=Path,
...
def main() -> None:
    ...
    (output_root / "content_manifest.json").write_text(
        json.dumps(
            build_prepared_content_manifest(output_root),
            indent=2,
            ensure_ascii=False,
        )
        + "\n",
