...
def expected_dataset_instances() -> dict[str, int]:
    output: dict[str, int] = {}
    for dataset_id in DATASET_IDS:
        ...
        output[dataset_id] = len(payload["annotations"])
    return output


EXPECTED_DATASET_INSTANCES = expected_dataset_instances()
...
def validate_plane_protocol_equivalence() -> None:
    path = STUDY_ROOT / "results" / "audits" / "plane_protocol_equivalence.json"
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("status") != "completed":
        raise AssertionError(f"Protocol equivalence audit is incomplete: {path}")
    if payload.get("equivalent_except_study_id") is not True:
        raise AssertionError(f"Small-vehicle protocol diverges from plane: {path}")
    if (
        payload.get("plane_protocol_hash_without_study_id")
        != payload.get("small_vehicle_protocol_hash_without_study_id")
    ):
        raise AssertionError(f"Protocol equivalence hashes differ: {path}")
...
def validate_rendered_documents(report_root: Path) -> None:
    ...
    expected_docx_tables = 7 if stem.startswith("isaid_small_vehicle_") else 6
...
def validate_prepared_test_sets() -> None:
    ...
    for dataset_id, expected_instances in EXPECTED_DATASET_INSTANCES.items():
        ...
        if annotated_image_ids != coco_image_ids:
            raise AssertionError(
                f"{dataset_id}: test set contains an image without a small-vehicle instance"
            )
...
def validate_analysis_outputs() -> None:
    ...
    expected_canonical_rows = (
        2 * conditions_per_reference * EXPECTED_DATASET_INSTANCES["isaid_small_vehicle"]
        + conditions_per_reference
        * EXPECTED_DATASET_INSTANCES["samrs_sota_small_vehicle"]
    )
    ...
    expected_reference_rows = {
        ("isaid_small_vehicle", "human"): (
            conditions_per_reference * EXPECTED_DATASET_INSTANCES["isaid_small_vehicle"]
        ),
        ("isaid_small_vehicle", "pseudo_sam1"): (
            conditions_per_reference * EXPECTED_DATASET_INSTANCES["isaid_small_vehicle"]
        ),
        ("samrs_sota_small_vehicle", "pseudo_sam1"): (
            conditions_per_reference
            * EXPECTED_DATASET_INSTANCES["samrs_sota_small_vehicle"]
        ),
    }
...
def main() -> None:
    validate_plane_protocol_equivalence()
