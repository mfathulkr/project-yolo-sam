...
from .io import portable_path, read_jsonl
from .paths import DATASETS, MODELS, REPO_ROOT, STRATA, prediction_path, reference_path
...
def qualitative_reference_figure(
    ...
    for row_index, (stratum, canonical_image_id) in enumerate(zip(STRATA[1:], image_ids, strict=True)):
        ...
        for column, reference_type in enumerate(("pseudo_sam1", "pseudo_sam2", "pseudo_sam3"), 2):
            ...
            union_iou = float(np.logical_and(mask, human).sum() / union) if union else 0.0
...
def paper_asset_source_paths(metrics: pd.DataFrame) -> list[Path]:
    """Return every non-analysis file read while building paper assets."""
    paths: list[Path] = []
    for dataset_id, source in DATASETS.items():
        paths.extend(
            (
        ...
        coco = COCO(str(source.coco_path))
        for canonical_image_id in _select_reference_examples(metrics, dataset_id):
            image_id = int(canonical_image_id.rsplit(":", 1)[-1])
            image_record = coco.loadImgs([image_id])[0]
            paths.append(source.images_root / str(image_record["file_name"]))
        for model in MODELS:
            prediction = prediction_path(source, model, "gt_bbox")
            paths.extend((prediction, prediction.with_name("manifest.json")))
        sam1_reference = _sam1_reference_path(dataset_id)
        paths.extend(
            (
                sam1_reference,
                sam1_reference.with_suffix(".manifest.json"),
            )
        )
        for teacher in ("sam2", "sam3"):
            reference = reference_path(dataset_id, teacher)
            paths.extend((reference, reference.with_suffix(".manifest.json")))
