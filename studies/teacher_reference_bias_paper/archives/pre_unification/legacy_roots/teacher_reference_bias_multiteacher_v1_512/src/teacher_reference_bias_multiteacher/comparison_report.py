...
def color_score(column: str, value: object, row: pd.Series) -> float | None:
    """Map table values to a common 0=bad, 1=good color score."""
    if column in HIGHER_IS_BETTER_COLUMNS:
        return numeric(value)
    if column not in LOWER_IS_BETTER_COLUMNS:
        return None
    ...
    try:
        ...
        return None
    if column in {"Empty masks", "Status mismatch"}:
        instances = float(row.get("Instances", 0))
        if instances <= 0:
            return None
        return max(0.0, min(1.0, 1.0 - raw / instances))
    if column == "Empty rate":
        return max(0.0, min(1.0, 1.0 - raw))
    if column == "Changes vs human":
        return max(0.0, min(1.0, 1.0 - raw / 3.0))
    return None
...
def empty_reference_table() -> pd.DataFrame:
    ...
    for dataset_id, source in DATASETS.items():
        for teacher in ("sam1", "sam2", "sam3"):
            ...
            predictions = pd.read_json(path, lines=True, typ="frame", dtype=False)
            actual_empty = predictions["predicted_mask_rle"].map(
                lambda rle: int(mask_utils.area(normalized_rle(rle))) == 0
            )
            status_empty = predictions["status"] == "empty_mask"
            empty = int(actual_empty.sum())
            rows.append(
                {
                    "Dataset": DATASET_LABELS[dataset_id],
                    ...
                    "Empty masks": empty,
                    "Empty rate": f"{empty / len(predictions):.3f}",
                    "Status mismatch": int((actual_empty != status_empty).sum()),
                }
            )
    return pd.DataFrame(rows)
...
def _docx_table(document: Document, frame: pd.DataFrame, title: str) -> None:
    ...
    for _, row in frame.iterrows():
        ...
        for index, column in enumerate(frame.columns):
            ...
            score = color_score(str(column), row[column], row)
    ...
    for index, (subtitle, frame) in enumerate(tables):
        ...
        for (row, column), cell in table.get_celld().items():
            if row == 0:
                ...
                frame_row = frame.iloc[row - 1]
                score = color_score(
                    str(frame.columns[column]),
                    frame_row.iloc[column],
                    frame_row,
                )
                if score is not None:
...
def write_comparison_report(
    *,
    ...
    worst_empty = empties.assign(
        _empty_rate=empties["Empty rate"].astype(float)
    ).sort_values(
        ["_empty_rate", "Dataset", "Teacher"],
        ascending=[False, True, True],
    ).iloc[0]
    worst_empty_summary = (
        f"En yüksek boş pseudo referans oranı {worst_empty['Dataset']} / "
        f"{worst_empty['Teacher']} koşulunda {worst_empty['Empty masks']}/"
        f"{worst_empty['Instances']} ({float(worst_empty['_empty_rate']):.1%}) "
        "değerindedir."
    )

    summary = [
        "GT-bbox diagonal hücreler coverage-aware identity control'dür: boş olmayan kendi-referansı 1,000, bilinen nesnedeki boş pseudo referansı 0,000 alır. Bunlar bağımsız model başarısı değildir.",
        "YOLO bbox koşulunda öğretmen avantajı ayrıca ölçülür; bu koşulda aday maske ile GT-bbox kaynaklı pseudo referans özdeş değildir.",
    ...
    definitions = [
        ...
        "Pseudo referans, SAM1/SAM2/SAM3'ün insan GT bbox istemiyle ürettiği maskedir. SAM3 için belirli nesne bbox segmentasyonuna uygun Sam3Tracker PVS arayüzü kullanılır.",
        "Bilinen insan GT instance için boş pseudo referans eksik anotasyondur. Boş tahminle eşleşse bile IoU, Dice, Precision ve Recall 0 kabul edilir.",
    ...
    limitations = [
        ...
        "GT-bbox diagonal hücreleri coverage-aware identity kontrolüdür; makalede ana performans sonucu olarak kullanılmamalıdır.",
        ...
        "Boş pseudo maskeler ayrı bir missing-label failure mode'udur. Bu değerlendirmede sıfır puanlanır ve oranları sonuçlarla birlikte verilir.",
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        _pdf_image_page(
            ...
            matrix_figure,
            "Satır: değerlendirilen model. Sütun: değerlendirme referansı. Diagonal hücreler coverage-aware identity kontrolüdür.",
        )
        _pdf_image_page(
            pdf,
