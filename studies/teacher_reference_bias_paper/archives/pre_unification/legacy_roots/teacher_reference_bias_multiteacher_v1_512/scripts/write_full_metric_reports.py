...
def empty_reference_stats(dataset_id: str, teacher: str) -> tuple[int, float]:
    ...
    predictions = pd.read_json(prediction_file, lines=True, typ="frame", dtype=False)
    ...
    mismatch = int((actual_empty != status_empty).sum())
    if mismatch:
        raise ValueError(
            f"{dataset_id}/{teacher}: {mismatch} status-mask uyuşmazlığı var"
        )
    empty = int(actual_empty.sum())
    return empty, empty / len(predictions)
...
def common_scope(dataset_id: str, target_label: str) -> tuple[str, ...]:
    return (
        ...
        "Low/High Mask Area ayrımı insan çizimli hedef instance maskelerinin "
        "alanları toplamına göre, testten önce dondurulmuş veri setine özgü "
        "eşikle yapılmıştır. Bu değer mask union alanı değildir; örtüşen piksel "
        "her instance için ayrı sayılabilir. Referans değişse bile stratum "
        "üyeliği değiştirilmemiştir.",
        "SAM1, SAM2 ve SAM3 aynı 512 görüntüde hem GT bbox hem seed 42 YOLO bbox istemiyle çalıştırılmıştır.",
        "SAM3 bbox koşulları, belirli bir nesneyi kutuyla segmentleyen Sam3Tracker PVS arayüzüyle çalıştırılmıştır; kavram örneği arayan PCS exemplar arayüzü kullanılmamıştır.",
        "İnsan, SAM1, SAM2 ve SAM3 değerlendirmelerinde aynı dondurulmuş model tahminleri kullanılmış; yalnız karşılaştırılan referans maske değiştirilmiştir.",
    ...
    *,
    ...
    return ReportSpec(
        ...
        context_bullets=(
            ...
            "GT-bbox diagonal hücre aynı tahmin ile kendi pseudo referansını karşılaştıran bir özdeşlik kontrolüdür. Boş olmayan referans 1,000; boş pseudo referans ise bilinen nesne için eksik etiket sayılarak 0,000 alır.",
        ...
        discussion_bullets=(
            f"{teacher.upper()} GT-bbox coverage-aware diagonal kontrolü {teacher_gt:.3f} değerindedir; bu bağımsız başarı değil referans kapsama ve özdeşlik kontrolüdür.",
            ...
            f"{empty_count:,} boş öğretmen maskesi bilinen GT nesnesi için eksik pseudo etikettir ve bu değerlendirmede 0 puanlanır. Bu nedenle diagonal ortalama aynı zamanda referans kapsamasını yansıtır.".replace(",", "."),
