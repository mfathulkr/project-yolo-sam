# Teacher Reference Bias Small Vehicle Deney Planı
...
### 3.1 iSAID Small Vehicle

- Hedef sınıf: `Small_Vehicle`
...
### 3.2 SAMRS SOTA Small Vehicle

- Hedef sınıf: `small-vehicle`
...
- Ana değerlendirme birimi: ayrı küçük araç örneği
...
- Detector ağırlıkları veya test tahminleri veri setleri arasında
  paylaşılmaz
- Eğitim görüntüsü ve nesne sayıları hazırlanmış split manifestlerinden rapora
  dinamik yazılır. Aynı epok ve hiperparametre ayarları farklı split
  büyüklüklerinde aynı optimizasyon adımı sayısı anlamına gelmediği için veri
  setleri arası detector farkı kontrollü referans etkisi olarak yorumlanmaz.
...
Strata tanımı nedeniyle testteki bütün görüntüler en az bir küçük araç içerir.
...
Low/High ayrımı, görüntüdeki referans küçük araç instance maskelerinin alanları
toplamının görüntü alanına oranına göre yapılır. Bu değer mask union alanı
değildir; örtüşen piksel her instance için ayrı sayılabilir. Veri setine özgü eşik aday havuzundan test
sonuçlarına bakılmadan önce dondurulur. Bu eşik model sonucuna göre
değiştirilmez.

Dondurulan eşikler iSAID için `0,0018463134765625`, SAMRS için
`0,0065670013427734` değeridir. iSAID eşiği model inference'ından önce resmi
test aday havuzundaki dört tabakanın her birinde en az 128 aday bırakacak
şekilde annotation dağılımından seçilir.
...
SAM3 bbox koşulları `Sam3Tracker` PVS arayüzüyle yürütülür; bu arayüz her
bbox'a tek belirli-instance maskesi döndürür. PCS visual-exemplar arayüzü bu
protokole eşdeğer olmadığı için kullanılmaz.
...
- Üst sınır: 100 epok
- Batch: 12
- Validation batch: 12; yalnız yoğun-sahne bellek güvenliği için, metrik ve
  örnek içeriği değişmeden
- Patience: 30
...
Her iSAID küçük araç örneğinde aşağıdakiler sabit kalır:
...
Önce her küçük araç ayrı hesaplanır, sonra nesneler eşit ağırlıkla ortalanır. Büyük
...
Bu oranlar ilgili eşiği geçen küçük araç örneklerinin payıdır; COCO AP veya mAP
değildir.

YOLO'nun kaçırdığı bir gerçek küçük araç boş maske olarak değerlendirilir ve o
...
Bilinen pozitif instance'ta pseudo referans boşsa, öğrenci tahmini de boş olsa
bile bütün maske skorları 0,0'dır. Böylece öğretmenin etiketlemediği nesne
yanlışlıkla kusursuz agreement olarak ödüllendirilmez.
...
1. `isaid_small_vehicle_human`
2. `isaid_small_vehicle_pseudo_sam1`
3. `samrs_sota_small_vehicle`
...
- Test instance sayıları hazırlanmış COCO anotasyonlarıyla birebir eşleşir
- Train/validation/test kaynak sahne kesişimi sıfır
- İki sabit seed 42 detector eğitimi ve iki gerçek COCO bbox testi
- Her veri setinde 3 GT-bbox ve 3 YOLO-bbox tahmin manifesti
- iSAID için insan ve pseudo, SAMRS için pseudo değerlendirme manifestleri
- Beklenen satır sayıları gerçek test instance sayılarından dinamik doğrulanır
