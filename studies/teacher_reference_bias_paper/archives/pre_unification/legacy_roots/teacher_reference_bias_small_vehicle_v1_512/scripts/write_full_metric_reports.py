...
def _training_image_count(dataset_id: str) -> int:
    metadata_path = STUDY_ROOT / "data" / "prepared" / dataset_id / "train" / "metadata.csv"
    if not metadata_path.is_file():
        raise FileNotFoundError(
            f"Eğitim görüntüsü sayısı için metadata bulunamadı: {metadata_path}"
        )
    return len(pd.read_csv(metadata_path, usecols=["image_id"]))


def _detector_training_scope() -> str:
    isaid_count = f"{_training_image_count('isaid_small_vehicle'):,}".replace(",", ".")
    samrs_count = f"{_training_image_count('samrs_sota_small_vehicle'):,}".replace(",", ".")
    return (
        "Detector protokolü aynı olsa da iSAID eğitim bölümü "
        f"{isaid_count}, SAMRS eğitim bölümü {samrs_count} görüntüdür; bu nedenle "
        "veri setleri arasındaki detector skoru farkı yalnız referans kaynağına "
        "bağlanan kontrollü bir etki değildir."
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "isaid_small_vehicle_pseudo_sam1",
...
def _common_scope(
    ...
    *,
    ...
    return (
        ...
        "Low/High Mask Area ayrımı, görüntüdeki "
        f"{area_reference_text} küçük araç instance maskelerinin alanları toplamının "
        "veri seti için testten önce dondurulan eşiğin altında veya üstünde "
        "olmasına göre yapılır. Bu değer mask union alanı değildir; örtüşen "
        "piksel her instance için ayrı sayılabilir.",
        "SAM1, SAM2 ve SAM3 aynı görüntülerde hem GT bbox hem YOLO bbox istemiyle çalıştırılmıştır.",
        "SAM3 bbox koşulu, her kutu için belirli bir instance maskesi üreten Sam3Tracker PVS arayüzüdür; kavram örneğine benzeyen tüm nesneleri arayan PCS visual-exemplar arayüzü kullanılmamıştır.",
...
def _isaid_reference_comparison(
    ...
    return (
        ...
        "Görüntü, küçük araç örneği, bbox ve model tahmini aynıdır; yalnız "
...
def isaid_human_spec(aggregates: pd.DataFrame) -> ReportSpec:
    ...
    return ReportSpec(
        study_id=STUDY_ID,
        dataset_id="isaid_small_vehicle",
        slug="isaid_small_vehicle_human",
        title="iSAID Small Vehicle Human Reference Full Metric Document",
        dataset_label="iSAID Small Vehicle",
        reference_sections=(
            ReferenceSection(
                reference_type="human",
        ...
        qualitative_image=FIGURES_DIR / "isaid_small_vehicle_gt_bbox_qualitative.png",
...
def isaid_pseudo_spec(aggregates: pd.DataFrame) -> ReportSpec:
    ...
    return ReportSpec(
        study_id=STUDY_ID,
        dataset_id="isaid_small_vehicle",
        slug="isaid_small_vehicle_pseudo_sam1",
        title="iSAID Small Vehicle SAM1 Pseudo Reference Full Metric Document",
        dataset_label="iSAID Small Vehicle",
        reference_sections=(
            ReferenceSection(
                reference_type="pseudo_sam1",
...
def samrs_spec(aggregates: pd.DataFrame) -> ReportSpec:
    ...
    return ReportSpec(
        study_id=STUDY_ID,
        dataset_id="samrs_sota_small_vehicle",
        slug="samrs_sota_small_vehicle",
        title="SAMRS SOTA Small Vehicle Full Metric Document",
        dataset_label="SAMRS SOTA Small Vehicle",
        reference_sections=(
            ReferenceSection(
                reference_type="pseudo_sam1",
        ...
        qualitative_image=(
            FIGURES_DIR / "samrs_sota_small_vehicle_gt_bbox_qualitative.png"
        ),
        scope_bullets=_common_scope(
...
def _samrs_comparison_if_available(
    ...
    return (
        ...
        "Bu ek tablo yalnız aynı küçük araç örnekleri hem SAMRS pseudo maskesi hem "
...
def main() -> None:
    ...
    specs = {
        "isaid_small_vehicle_human": isaid_human_spec(aggregates),
        "isaid_small_vehicle_pseudo_sam1": isaid_pseudo_spec(aggregates),
        "samrs_sota_small_vehicle": samrs_spec(aggregates),
    }
    for report_id, spec in specs.items():
        if args.report not in ("all", report_id):
            continue
        if report_id.startswith("isaid_small_vehicle_"):
