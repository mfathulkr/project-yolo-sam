...
from yolo_sam.runtime.manifest import validate_completed_run_manifest
...
def collect_canonical_metrics(
    ...
    *,
    detector_seeds: set[int] | None = None,
) -> tuple[pd.DataFrame, list[Path]]:
    metric_files = _preferred_metric_files(evaluation_root)
    if detector_seeds is not None:
        metric_files = [
            path
            for path in metric_files
            if (seed := _seed_from_path(path)) is None or seed in detector_seeds
        ]
    if not metric_files:
    ...
    for path in metric_files:
        ...
        if manifest.get("stage") != "matched_prediction_evaluation":
            raise ValueError(f"Unexpected evaluation stage: {manifest_path}")
        ...
        if parameters.get("primary_granularity") != "instance":
            raise ValueError(f"Unexpected primary metric granularity: {manifest_path}")
        output_fingerprint = manifest.get("output_file_fingerprints", {}).get(
            "metrics_instance"
        )
        if not isinstance(output_fingerprint, dict):
            raise ValueError(f"Missing metrics output fingerprint: {manifest_path}")
        if output_fingerprint.get("sha256") != sha256_file(path):
            raise ValueError(f"Metrics output hash mismatch: {path}")
        dependency_files.extend((path, manifest_path))
    ...
    return metrics, dependency_files
...
def collect_detector_metrics(
    ...
    *,
    ...
    if detector_seeds is not None:
        files = [path for path in files if _seed_from_path(path) in detector_seeds]
    rows: list[dict[str, object]] = []
    for path in files:
...
def write_analysis_manifest(
    ...
    *,
    ...
    payload = {
        ...
        "inputs": [
            {
                "path": input_path.resolve().relative_to(REPO_ROOT).as_posix(),
        ...
        "outputs": [
            {
                "path": output_path.resolve().relative_to(REPO_ROOT).as_posix(),
