...
def main() -> None:
    ...
    temporary_unmatched_path = output_root / "unmatched_detector_predictions.jsonl.tmp"
    ...
    temporary_unmatched_path.unlink(missing_ok=True)
    ...
    run_id = (
        ...
        f"yolo-bbox-seed-{args.seed}-{config_hash[:12]}"
    )
    ...
    temporary_predictions_path.write_text("", encoding="utf-8")
    temporary_unmatched_path.write_text("", encoding="utf-8")
    prediction_mode = "a"
    unmatched_mode = "a"
    matched_count = 0
    missing_count = 0
    ...
    temporary_unmatched_path.replace(unmatched_path)
    manifest["outputs"] = {
