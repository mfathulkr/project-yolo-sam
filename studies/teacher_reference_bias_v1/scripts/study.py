...
DEFAULT_PROTOCOL = Path("studies/teacher_reference_bias_v1/configs/protocol.yaml")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    audit_parser.add_argument(
        ...
        type=Path,
        default=Path(
            "studies/teacher_reference_bias_v1/results/dataset_audits"
        ),
    )
    ...
    preflight_parser.add_argument(
        "--output",
        type=Path,
        default=Path("studies/teacher_reference_bias_v1/results/preflight.json"),
    )
    ...
    parity_parser = subparsers.add_parser(
        "prediction-parity",
        help="Snapshot or verify GT-bbox mask parity after model pinning.",
    )
    add_protocol_argument(parity_parser)
    parity_parser.add_argument(
        "--mode",
        choices=("snapshot", "verify"),
        required=True,
    )
    parity_parser.add_argument("--dataset", type=Path, action="append")
    parity_parser.add_argument("--output", type=Path)
    add_force_argument(parity_parser)
    ...
    paper_parser = subparsers.add_parser(
        "paper",
        help="Build the source-backed Markdown, DOCX, and six-page PDF paper.",
    )
    add_protocol_argument(paper_parser)
    ...
    paper_parser.add_argument(
        "--allow-partial",
        action="store_true",
        help="Build a visibly marked draft before all frozen results exist.",
    )

    finalize_parser = subparsers.add_parser(
        "finalize",
        help="Verify completeness, backfill hashes, and write the reproducibility appendix.",
    )
    add_protocol_argument(finalize_parser)
    finalize_parser.add_argument("--dataset", type=Path, action="append")
...
def prepare_command(args: argparse.Namespace) -> int:
    ...
    if skip_completed(completion_path, force=args.force):
        return 0
    script_by_profile = {
        "isaid": "prepare_matched_isaid_plane.py",
        "samrs_sota": "prepare_matched_samrs_sota_plane.py",
    }
    try:
...
def prediction_parity_command(args: argparse.Namespace) -> int:
    command_args = [
        "--mode",
        args.mode,
        "--protocol",
    ...
    for dataset in args.dataset or []:
        command_args.extend(["--dataset", str(project_path(dataset))])
    if args.output is not None:
        command_args.extend(["--output", str(project_path(args.output))])
    if args.force:
        command_args.append("--force")
    run_script("audit_pinned_prediction_parity.py", command_args)
    return 0
...
def paper_command(args: argparse.Namespace) -> int:
    ...
    command_args = [
        ...
        str(study_root(protocol)),
        "--output-dir",
        str(project_path(args.output_dir)),
    ]
    if args.allow_partial:
        command_args.append("--allow-partial")
    run_script("write_teacher_reference_bias_paper.py", command_args)
    return 0
...
def finalize_command(args: argparse.Namespace) -> int:
    command_args = [
        ...
        str(project_path(args.protocol)),
        "--paper-dir",
        str(project_path(args.paper_dir)),
        "--appendix",
        str(project_path(args.appendix)),
    ]
    for dataset in args.dataset or []:
        command_args.extend(["--dataset", str(project_path(dataset))])
    run_script("finalize_matched_study.py", command_args)
    return 0
...
def main() -> int:
    ...
    if args.command == "prediction-parity":
        return prediction_parity_command(args)
