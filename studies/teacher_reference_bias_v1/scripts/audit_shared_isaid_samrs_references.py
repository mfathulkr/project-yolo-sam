...
def load_isaid_plane_annotations(
    isaid_root: Path,
) -> tuple[
    dict[str, Path],
...
def main() -> None:
    ...
    ) = load_isaid_plane_annotations(args.isaid_root)
    ...
    for image_record in samrs_coco.loadImgs(sorted(samrs_coco.getImgIds())):
        ...
        for samrs_index, human_index, bbox_iou in matches:
            ...
            instance_id = (
                f"samrs_sota_plane:{image_id}:{int(samrs_annotation['id'])}"
            )
    ...
    if not match_rows:
        raise ValueError("No SAMRS-to-iSAID plane instances were matched")
