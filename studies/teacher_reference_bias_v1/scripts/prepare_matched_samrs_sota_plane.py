...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare a source-scene-safe matched SAMRS SOTA plane corpus."
    )
    parser.add_argument(
        "--protocol",
        type=Path,
    ...
    parser.add_argument(
        "--dataset",
        type=Path,
        default=STUDY_ROOT / "configs" / "datasets" / "samrs_sota_plane.yaml",
    )
    parser.add_argument(
        "--eval-per-stratum",
        type=int,
...
def main() -> None:
    ...
    summaries["test"] = create_balanced_eval_split(
        ...
        area_threshold=str(protocol.evaluation["area_threshold"]),
