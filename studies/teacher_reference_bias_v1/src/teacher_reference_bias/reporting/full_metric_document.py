    ...
    for model in ("sam1", "sam2", "sam3"):
        for bbox_source in ("gt_bbox", "yolo_bbox"):
            ...
            if bbox_source == "gt_bbox":
                ...
                if seed_count < 2:
                    raise ValueError(
                        f"Expected repeated YOLO-bbox seeds, found {seed_count}"
                    )
...
def build_isaid_reference_effect_table(
    ...
    selected = aggregates[
        (aggregates["dataset_id"] == "isaid_plane")
...
def build_qualitative_examples(
    ...
    with Image.open(source_path) as source:
        ...
        for (stratum, title), (y0_ratio, y1_ratio) in zip(
            QUALITATIVE_LABELS,
            row_ranges,
            strict=True,
        ):
            ...
            for label, (x0_ratio, x1_ratio), (x, y) in zip(
                panel_labels,
                column_ranges,
                panel_positions,
                strict=True,
                ...
                panel = source.crop(
                    (
                        round(width * x0_ratio),
                        round(height * y0_ratio),
    ...
    *,
    ...
    for section in spec.reference_sections:
        ...
        for stratum, stratum_label in STRATA:
            _add_docx_table(
                document,
                tables[(section.reference_type, stratum)],
                title=stratum_label,
                note=(
                    f"Referans: {section.title}. Overall 128 görüntü; her "
                    "overlap × mask-area tablosu 32 görüntüdür. YOLO bbox "
                    "değerleri üç seed ortalaması ± standart sapmadır."
                ),
            )
        ...
        for section in spec.reference_sections:
            for stratum, stratum_label in STRATA:
                _add_pdf_table(
                    ...
                    title=stratum_label,
                    note=(
                        f"Referans: {section.title}. Overall 128 görüntü; alt "
                        "grupların her biri 32 görüntüdür. YOLO bbox: üç "
                        "bağımsız eğitim ortalaması ± standart sapma."
                    ),
                )
        _add_pdf_table(
            pdf,
        ...
        for stratum, stratum_label in STRATA:
            lines.extend(
                [
                    f"### {stratum_label}",
                    "",
                    _markdown_table(tables[(section.reference_type, stratum)]),
                    "",
                ]
            )
    lines.extend(
...
def write_report(
    *,
    ...
    manifest = {
        "schema_version": 1,
        "study_id": "teacher_reference_bias_v1",
        "dataset_id": spec.dataset_id,
