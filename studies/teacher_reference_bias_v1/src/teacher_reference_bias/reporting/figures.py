...
def shared_human_reference_figure(
    ...
    axis.set_title(
        "Aynı 1.033 tahmin, iki referans: üretici-referans skor artışı"
    )
...
def _select_representative_instances(
    ...
    *,
    ...
    for stratum in STRATUM_ORDER[1:]:
        stratum_rows = selected[selected["stratum"] == stratum].copy()
        if stratum_rows.empty:
            raise ValueError(f"No representative candidate for {dataset_id}/{stratum}")
        median = float(stratum_rows["iou"].median())
        stratum_rows["distance_to_median"] = (
            stratum_rows["iou"].astype(float) - median
        ).abs()
        row = stratum_rows.sort_values(
