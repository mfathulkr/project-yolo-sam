...
from yolo_sam.evaluation.visualization import save_pipeline_comparison_figure
...
BASE_PIPELINES = [
    ...
    ("sam3_hybrid_yolo", "SAM3 hybrid YOLO bbox", "iou_sam3_hybrid_yolo", "sam3_hybrid_yolo_output_dir"),
]
...
PIPELINE_COLORS = {
    ...
    "sam3_hybrid_yolo": (255, 140, 40),
    "yolo_sam2": (0, 255, 255),
    "gt_box_sam2": (80, 180, 255),
    ...
    "gt_box_sam1": (255, 210, 90),
    "yolo_sam1": (110, 210, 255),
}
...
def configured_pipelines(config: dict) -> list[tuple[str, str, str, str]]:
    ...
    if "gt_box_sam2_output_dir" in config.get("paths", {}):
        pipelines.append(("gt_box_sam2", "GT bbox + SAM2", "iou_gt_box_sam2", "gt_box_sam2_output_dir"))
    if "sam1_yolo_output_dir" in config.get("paths", {}):
        pipelines.append(("yolo_sam1", "YOLO + SAM1", "iou_yolo_sam1", "sam1_yolo_output_dir"))
    if "sam1_gt_box_output_dir" in config.get("paths", {}):
        pipelines.append(("gt_box_sam1", "GT bbox + SAM1", "iou_gt_box_sam1", "sam1_gt_box_output_dir"))
...
def summarize_long_metrics(
    ...
    if {"yolo_sam2", "yolo_sam1"}.issubset(pipeline_labels):
        comparisons.append(("yolo_sam2", "yolo_sam1", "YOLO + SAM2 - YOLO + SAM1"))
    if {"gt_box_sam2", "gt_box_sam1"}.issubset(pipeline_labels):
        comparisons.append(("gt_box_sam2", "gt_box_sam1", "GT bbox + SAM2 - GT bbox + SAM1"))
