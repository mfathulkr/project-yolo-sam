...
def main() -> None:
    ...
    reference_cfg = config["training_time_reference"]
    baseline_results_value = reference_cfg.get("baseline_results_csv")
    if baseline_results_value:
        baseline_results_path = resolve_path(baseline_results_value)
        if baseline_results_path.exists():
            ...
            baseline_seconds_per_epoch = (
                float(baseline_results["time"].iloc[-1]) / baseline_epochs
            )
            baseline_source = str(baseline_results_path)
        else:
            baseline_seconds_per_epoch = float(
                reference_cfg["baseline_seconds_per_epoch"]
            )
            baseline_source = (
                f"configured_scalar_missing_csv:{baseline_results_path}"
            )
    else:
        baseline_seconds_per_epoch = float(
            reference_cfg["baseline_seconds_per_epoch"]
        )
        baseline_source = "configured_scalar"
