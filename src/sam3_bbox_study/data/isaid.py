...
def convert_isaid_split_to_tiles(
    ...
    for source_image in tqdm(data["images"], desc=f"Preparing iSAID {split_name}"):
        ...
        for tile_y in tile_starts(height, tile_size, stride, include_edge_tiles):
            for tile_x in tile_starts(width, tile_size, stride, include_edge_tiles):
                ...
                for annotation in source_annotations:
                    ...
                    segmentations, bbox, area = clipped
    ...
    categories = [{"id": 1, "name": merged_category_name, "supercategory": "vehicle"}]
    save_json(
        {
            "info": {"description": f"iSAID tiled {merged_category_name} subset"},
            "licenses": [],
            "images": coco_images,
