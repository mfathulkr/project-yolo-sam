...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--presentation-dir",
        type=Path,
        default=ROOT.parent / "presentation_isaid_vehicle_sam3_sam2_study",
    )
...
def write_markdown_report(
    ...
    if "visualization" not in selected.columns:
        ...
        selected["visualization"] = selected.apply(
            lambda row: f"results/isaid_vehicle_visualizations_sam3_triplet/{row['stratum']}__{Path(row['file_name']).stem}.png",
            axis=1,
        )
